...
#                           'benchmark').
#   spp.bloomberg_serie /   copied as-is into the ported tables,
#   spp.bloomberg_dato      preserving serie_id (the schema uses
#                           GENERATED BY DEFAULT for exactly this),
#                           then the identity sequence is realigned.
#
...
def migrate_bloomberg(src, dest, schema: str, dry_run: bool) -> tuple[int, int]:
    """Copies bloomberg_serie + bloomberg_dato preserving serie_id."""
    try:
        series = src.execute(
    ...
    for s in series:
        ...
        r = dest.execute(
            ...
            INSERT INTO bloomberg_serie (
                serie_id, ticker, ticker_clave, campo, intervalo, descripcion,
                moneda, fecha_inicio, activa, ultima_fecha, ultimo_intento,
                ultimo_resultado, ultimo_error, puntos, creada_en
            ) VALUES (%(serie_id)s, %(ticker)s, %(ticker_clave)s, %(campo)s,
    ...
    for d in cur:
        ...
        r = dest.execute(
            ...
            VALUES (%(serie_id)s, %(fecha)s, %(valor)s)
    ...
    if not dry_run:
        # Realign the identity sequence past the migrated ids.
        dest.execute(
            """
            SELECT setval(
                pg_get_serial_sequence('bloomberg_serie', 'serie_id'),
                GREATEST((SELECT COALESCE(MAX(serie_id), 0) FROM bloomberg_serie), 1)
            )
            """
        )
