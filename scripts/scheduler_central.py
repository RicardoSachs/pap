...
def job_check_sbs():
    ...
    missing = []
    for f_name, f_dict in SBS_FILES.items():
        path = find_latest_file(f_dict["folder"], run_date)
        if path is None or path.stem[:8] != date_prefix:
            missing.append(f_name)
        else:
            logger.info(f"  {f_name}: OK ({path.name})")
...
if scraper_enabled():
    ...
    # acquire at 08:30, ingest at 08:50 with no check needed
    scheduler.add_job(
        job_acquire_sbs,
        CronTrigger(day_of_week="mon-fri", hour=8, minute=30), #hour=8, minute=0),
        id="acquire_sbs",
    ...
    # check at 08:40, ingest at 08:50
