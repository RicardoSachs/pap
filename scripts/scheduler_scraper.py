...
def job_acquire_sbs(legacy: Optional[bool] = False):
    """
    ...
    Files land on network share for central PC ingestion at 08:50.
    """
    ...
    # Check if files already in path
    # from src.scrapers.sbs import find_latest_file, SBS_FILES
    # date_prefix = run_date.strftime('%Y%m%d')
    # already_present = all(
    #     find_latest_file(f['folder'], run_date) is not None
    #     and find_latest_file(f['folder'], run_date).stem[:8] == date_prefix
    #     for f in SBS_FILES
    # )

    from src.scrapers.sbs import all_files_present, VP_SBS_DIR_LEGACY, SBS_FILES_LEGACY

    raw_dir = VP_SBS_DIR_LEGACY if legacy else None
    sbs_files = SBS_FILES_LEGACY if legacy else None

    already_present = all_files_present(sbs_files=sbs_files)
