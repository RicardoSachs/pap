...
CREATE TABLE IF NOT EXISTS dim_portfolio (
    ...
    source           TEXT NOT NULL,            -- fms | sbs | bloomberg
...
CREATE INDEX IF NOT EXISTS ix_dim_portfolio_source
    ON dim_portfolio (source);

CREATE INDEX IF NOT EXISTS ix_dim_portfolio_status
    ON dim_portfolio (status);

CREATE INDEX IF NOT EXISTS ix_dim_portfolio_parent
    ON dim_portfolio (parent_entity_id);
