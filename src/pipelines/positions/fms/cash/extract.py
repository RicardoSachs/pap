# src/pipelines/positions/fms/cash/extract.py
# ---------------------------------------------------------------
# Executes the FMS cash query and returns a raw DataFrame with
# vendor-native (PascalCase) column names. All translation to
# staging shape happens in transform.py, not here.
#
# The query lives in ./queries/cash.sql with two positional ?
# placeholders for start_date and end_date, both INT yyyymmdd.
# This module converts Python date objects to that format at the
# boundary.
#
# Range size validation lives here (not in the SQL): reject ranges
# wider than MAX_RANGE_DAYS unless force=True.
# ---------------------------------------------------------------
...
QUERY_PATH = Path(__file__).parent / "queries" / "cash.sql"
MAX_RANGE_DAYS = 90


def extract(start_date: date, end_date: date, force: bool = False) -> pd.DataFrame:
    """
    Pull FMS cash balances for the given inclusive date range.

    Returns a DataFrame with vendor-native (PascalCase) column names.
    Empty DataFrame if no rows in range.
    ...
    logger.info(
        f"executing FMS cash query: {start_date} to {end_date} "
    ...
    if not rows:
        logger.warning(f"FMS cash query returned zero rows for {start_date}..{end_date}")
        return pd.DataFrame(columns=columns)

    df = pd.DataFrame.from_records(rows, columns=columns)
    logger.info(f"FMS cash extract: {len(df)} rows fetched")
    return df
