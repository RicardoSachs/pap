...
def run_from_stg(batch_id: str) -> None:
    ...
    DB access. Note the staging PK is (codigo_fondo, codigo_iso_moneda, date)
    and does NOT include batch_id, and load_staging overwrites batch_id on
    conflict — so a grain row always carries its most recent batch_id. This
    resolves the common "rebuild what I just loaded" case; an older batch_id
    whose grain was later re-staged returns no rows.
    """
...
def _new_batch_id() -> str:
    return "fms_cash_" + datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
...
def _log_null_counts(df: pd.DataFrame, stage: str) -> None:
    ...
    row missing SaldoContable/MontoTotalSoles) is visible before it trips a
    NOT NULL insert. Silent when nothing is null.
    """
    if df.empty:
        return
