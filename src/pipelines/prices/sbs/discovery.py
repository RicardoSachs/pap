# src/pipelines/prices/sbs/discovery.py
# ---------------------------------------------------------------
# Shared SBS universe DISCOVERY / registration logic.
# Called as Step 1 in each SBS pipeline run() before _classify().
#
# HISTORY: this module lived at registry.py until the 2026-09 scheduler
# refactor reused that filename for the pipeline-dispatch table
# (run_sbs_pipelines), clobbering it. Restored from git (fac028a) under
# this name; registry.py is now ONLY the dispatch table.
#
...
def _register_securities(
    ...
    # Unique instruments in file. `date` is carried through when present
    # (the set-based backfill registration frame has it - latest date each
    # code appeared) so identifier observations get per-code seen dates.
    # That is what keeps primary-code promotion chronological instead of
    # processing-order-dependent.
    cols = ["codigo_sbs", "isin", "tipo_instrumento", "emisor", "moneda"]
    ...
    if not existing_instruments.empty:
        with get_connection() as conn:
            for _, row in existing_instruments.iterrows():
                ...
                if entity_id:
                    seen = _row_seen_date(row, run_date)
                    # Refresh the code's observation window so primary
                    # promotion tracks the latest-seen code, and record a
                    # changed ISIN as an alias (same code, new ISIN - e.g.
                    # restructurings; X-ISINs stay under id_type='isin_x').
                    upsert_entity_identifier(
                        conn, entity_id, "codigo_sbs", codigo_sbs, "sbs",
                        is_primary=True, seen_date=seen,
                    )
                    isin = _clean(row.get("isin"))
                    if isin and not _is_x_isin(isin):
                        upsert_entity_identifier(
                            conn, entity_id, "isin", isin, "internal",
                            is_primary=False, seen_date=seen,
                        )
...
def _register_normal_instrument(
    ...
    # Always store codigo_sbs on the entity (new or existing). Identifiers
    # are MULTI-VALUED: a re-coded instrument (resolved via ISIN above) gets
    # the new code as an ADDITIONAL row - the latest-seen code becomes
    # primary and the old one persists as an alias for historical joins.
    upsert_entity_identifier(
        conn, entity_id, "codigo_sbs", codigo_sbs, "sbs",
        is_primary=True, seen_date=seen,
    )
...
def _register_x_isin_instrument(
    ...
    if isin_x:
        upsert_entity_identifier(
            conn, entity_id, "isin_x", isin_x, "sbs",
            is_primary=False, seen_date=seen,
        )
    ...
    if isin_x:
        ...
        if parent_entity:
            ...
            upsert_entity_identifier(
                conn, entity_id,
                id_type="parent_entity_id",
                id_value=str(parent_entity),
                source="sbs",
                is_primary=True,    # latest resolution is the current hint
                seen_date=seen,
            )
            logger.debug(
    ...
    with get_connection() as conn:
        for _, row in pairs.iterrows():
            ...
            for id_type, id_value in [
                ...
                upsert_entity_identifier(
                    conn, entity_id, id_type, id_value, "sbs",
                    is_primary=False, seen_date=run_date,
                )
...
def _row_seen_date(row, run_date: date) -> date:
    """
    Observation date for identifier rows: the row's own `date` when the
    frame carries one (the set-based backfill registration frame does -
    the latest date each code appeared), else run_date. Normalized to a
    plain date so primacy comparisons in upsert_entity_identifier are
    type-safe (pd.Timestamp is a datetime subclass).
    """
    try:
        d = row.get("date")
    ...
    if isinstance(d, datetime):
        return d.date()
    return d if isinstance(d, date) else run_date
