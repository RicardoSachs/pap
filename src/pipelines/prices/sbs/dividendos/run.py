...
#
# Key differences:
#   - FILE_TYPE = "dividendos" (1 field: DIV_ADJ_FACTOR)
#   - Staging table: stg_prices_sbs_dividendos
#   - No dim_security updates
#   - Most files will be empty (dividends are infrequent)
# ---------------------------------------------------------------
...
def run(run_date: Optional[date] = None, series_override: Optional[list[dict]] = None) -> None:
    ...
    if not securities:
        logger.warning("No active dividendos series. Exiting.")
        return
    ...
    with get_connection() as conn:
        cur = conn.execute(
            "SELECT * FROM stg_prices_sbs_dividendos WHERE date = %s ORDER BY loaded_at DESC",
            (run_date,),
        )
        rows = cur.fetchall()
    ...
    logger.info("=== prices/sbs/dividendos complete ===")
