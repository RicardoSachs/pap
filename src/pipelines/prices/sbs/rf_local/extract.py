...
RAW_COLUMNS = {
    ...
    "PRECIO LIMPIO (MONTO)": "precio_limpio_monto", "PRECIO LIMPIO (%)": "precio_limpio_pct",
    "PRECIO SUCIO (MONTO)": "precio_sucio_monto", "PRECIO SUCIO (%)": "precio_sucio_pct",
    "INTER&Eacute;S CORRIDO (MONTO)": "interes_corrido_monto", "TIR %": "tir", "SPREADS": "spreads",
    "FECHA EMISI&Oacute;N": "fecha_emision", "FECHA VENCIMIENTO": "fecha_vencimiento",
    "TASA CUP&Oacute;N": "tasa_cupon", "MARGEN LIBOR (%)": "margen_libor",
    "TIR % SIN OPCIONES": "tir_sin_opciones", "RATING": "rating",
    "&Uacute;LTIMO CUP&Oacute;NULTIMO CUP&Oacute;N": "ultimo_cupon", "PR&Oacute;XIMO CUP&Oacute;N": "proximo_cupon",
    "DURACI&Oacute;N": "duracion", "VARIACI&Oacute;N PRECIO LIMPIO": "variacion_precio_limpio",
    "VARIACI&Oacute;N PRECIO SUCIO": "variacion_precio_sucio", "VARIACI&Oacute;N TIR": "variacion_tir",
}
    ...
    for _, row in df.iterrows():
        cur = conn.execute(
            ...
            INSERT INTO stg_prices_sbs_rf_local (
                ...
                variacion_precio_sucio, variacion_tir, date, loaded_at
            ) VALUES (
                %s, %s, %s, %s, %s, %s,
                %s, %s, %s, %s,
                %s, %s, %s, %s, %s,
            ...
            (
                ...
                _f(row.get("tir")),
                _f(row.get("spreads")),
                _f(row.get("tir_sin_opciones")),
                _f(row.get("duracion")),
                _f(row.get("variacion_precio_limpio")),
                _f(row.get("variacion_precio_sucio")),
                _f(row.get("variacion_tir")),
                row["date"],
                row["loaded_at"],
            ),
        )
