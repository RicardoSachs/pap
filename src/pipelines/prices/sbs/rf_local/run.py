...
# Same architecture as vector_completo/run.py.
#
# Key differences from vector_completo:
#   - FILE_TYPE = "rf_local" (12 fields per instrument)
#   - Staging table: stg_prices_sbs_rf_local (typed columns)
#   - psycopg3 returns date objects — no string conversion needed
# ---------------------------------------------------------------
...
def run(run_date: Optional[date] = None, series_override: Optional[list[dict]] = None) -> None:
    ...
    # psycopg3 returns typed data (date objects, floats) — no conversion needed
    with get_connection() as conn:
        cur = conn.execute(
            "SELECT * FROM stg_prices_sbs_rf_local WHERE date = %s ORDER BY loaded_at DESC",
            (run_date,),
        )
        rows = cur.fetchall()
    ...
    with get_connection() as conn:
        for sec in securities:
            ...
            if rows.empty:
                ...
                # psycopg3 returns date objects — handle both types
                last_loaded = (
