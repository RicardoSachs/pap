# src/pipelines/prices/sbs/tipo_cambio/run.py
# ---------------------------------------------------------------
# Fact pipeline entry point for SBS tipo_cambio (FX rates).
# Same architecture as rf_local/run.py.
#
# Key differences:
#   - FILE_TYPE = "tipo_cambio" (4 fields: PX_BID, PX_ASK, CHG_BID, CHG_ASK)
#   - Staging table: stg_prices_sbs_tipo_cambio
#   - FX instruments keyed by (moneda_nocional, moneda_contraparte, fuente)
#   - No dim_security updates
#   - Registry uses _register_fx (different from bond registration)
# ---------------------------------------------------------------
...
from src.pipelines.prices.sbs.tipo_cambio.extract import extract, load_stg
from src.pipelines.prices.sbs.tipo_cambio.transform import transform
from src.pipelines.prices.sbs.tipo_cambio.loader import load_facts, load_dims

logger = logging.getLogger(__name__)
FILE_TYPE = "tipo_cambio"
TC_FIELDS = set(FILE_TYPE_FIELDS[FILE_TYPE])


def run(run_date: Optional[date] = None, series_override: Optional[list[dict]] = None) -> None:
    run_date = run_date or date.today()
    logger.info(
        f"=== prices/sbs/tipo_cambio | run_date={run_date} | "
    ...
    # Step 2: discover and register new FX pairs
    if not raw_df.empty:
    ...
    with get_connection() as conn:
        cur = conn.execute(
            "SELECT * FROM stg_prices_sbs_tipo_cambio WHERE date = %s ORDER BY loaded_at DESC",
            (run_date,),
        )
        rows = cur.fetchall()
