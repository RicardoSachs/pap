...
def run(run_date: Optional[date] = None, series_override: Optional[list[dict]] = None) -> None:
    ...
    with get_connection() as conn:
        cur = conn.execute(
            "SELECT * FROM stg_prices_sbs_vector_completo WHERE date = %s ORDER BY loaded_at DESC",
            (run_date.isoformat(),),
        )
        rows = cur.fetchall()
    ...
    with get_connection() as conn:
        for sec in securities:
            ...
            if rows.empty:
                ...
                last_loaded = max_date if isinstance(max_date, date) else date.fromisoformat(max_date) 
